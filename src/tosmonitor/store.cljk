(ns tosmonitor.store
  ...
  This repo archives exactly ONE company (Deutsche Bahn AG, LEI
  52990063S23N13HU4E98) -- unlike `commitledger.store`'s multi-application
  directory, this Store holds a SINGLE `:company` record and a SINGLE
  `:baseline` record (the latest archived Beforderungsbedingungen/
  Conditions-of-Carriage snapshot this actor treats as ground truth to
  compare candidates against), plus the append-only audit ledger and
  change-proposal draft history.

  NOTE: this company's archived doc-type is `:terms-of-carriage`, the FIRST
  use of this vocabulary value in this actor family (see `tosmonitor.
  registry/known-doc-types`, which was corrected during the first
  validation batch to reflect the archive family's real observed
  vocabulary rather than a guessed one -- ADR-2607242000).
...
(def ^:private demo-full-text
  "Beforderungsbedingungen\nDiese Beforderungsbedingungen gelten fur die Befoerderung von Personen mit der Deutsche Bahn AG. Mit der Nutzung unserer Verkehrsmittel erkennen Sie diese Bedingungen an. Es gilt deutsches Recht.")

(defn demo-baseline []
  {:full-text    demo-full-text
   :source-url   "https://www.bahn.de/agb"
   :retrieved-at "2026-07-10T06:26:33Z"
